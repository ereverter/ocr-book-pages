...
def binarization(image, blur_type, thresh_type, thresh_values):
    if blur_type == "median":
        image = cv2.medianBlur(image, 5)
    elif blur_type == "gaussian":
        image = cv2.GaussianBlur(image, (5, 5), 0)
    
    min_thresh, max_thresh = thresh_values
...
if __name__ == "__main__":
    ...
    for filename in os.listdir(args.src_folder):
        if filename.endswith(('.png', '.jpg', '.jpeg', '.tiff')):
            ...
            # Binarization
            bin_image = binarization(gray_image, args.blur_type, args.thresh_type, (args.min_thresh, args.max_thresh))
            
            # Noise Removal
            denoised_image = noise_removal(bin_image, args.noise_kernel, args.noise_iter)
            
            # Thin Font
            thin_image = thin_font(denoised_image, args.erode_kernel, args.erode_iter)
            
            # Thick Font
            thick_image = thick_font(thin_image, args.dilate_kernel, args.dilate_iter)
